...
- `Pseudos`
  ...
  Extends:
    - `AdvancedPseudos`
      ...
      Function-like pseudos e.g. `:not(:first-child)`. The string literal contains the value excluding the parenthesis:
...
`CSS.PropertiesFallback` instead of `CSS.Properties` will add the possibility to use any property value as an array of
values.
...
not **not** added by default in `CSS.Properties`. To allow both of them, you can simply extend with
`CSS.PropertiesHyphen` or/and `CSS.PropertiesHyphenFallback`.
...
- **No longer compatible with version 2**  
  Conflicts may occur when both version ^3.2.0 and ^2.0.0 are installed. Potential fix for Npm would be to force
